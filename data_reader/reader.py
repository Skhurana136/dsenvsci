...
def readTecfile(filename):
    ...
    line_ex = (
        fid.readline()
    )  # check if readline includes or excludes new line character. chose function that removes new line character
    ...
    D = np.zeros(
        [dataPoints, len(Headers), numberOfTimeSteps]
    )  # Initializing np.darray
