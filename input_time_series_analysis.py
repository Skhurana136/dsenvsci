...
for Reg in Regimes:
    ...
    for p in Tforfpre[1:]:
        print (p)
        for k in plotg:
            fig, axes = plt.subplots(nrows = 4, ncols = 3, figsize = [20,15], sharey = True, sharex = True)
            plt.suptitle(p+k)
            for t in Trial:
                print (t)
                ...
                normavgconcout[:,gvarnames.index(k)] = (conctime0[:,yout,gvarnames.index(k)] - np.mean(conctime0[:,yout,gvarnames.index(k)]))/np.std(conctime0[:,yout,gvarnames.index(k)])
...
#                axes.flat[Trial.index(t)].semilogy(freq[L], PSD[L], color = 'r', LineWidth = 2, label = 'Noisy')
            ...
            plt.savefig(directory+p+"_med_var_fft_trimmed_"+k+"_chem.png", dpi = 300, bbox_inches='tight', pad_inches = 0.01)
...
for Reg in Regimes:
    ...
    for p in Tforfpre[1:]:
        print (p)
        for k in plotg:
            fig, axes = plt.subplots(nrows = 8, ncols = 6, figsize = [20,15], sharey = True, sharex = True)
            plt.suptitle(p+k)
            for t in Trial:
                print (t)
                path = directory+p+fpre+str(t)+fsuf+fpre+str(t)
                df0, massendtime0, masstime0, conctime0, Velocity0, Headinlettime0 = sta.calcconcmasstime(t, Het[Trial.index(t)], Anis[Trial.index(t)], gw, directory+p, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames)
                freqs, times, spect = signal.spectrogram(conctime0[:,-1,gvarnames.index(k)], fsample)
#                axes.flat[Trial.index(t)].imshow(spect, aspect = 'auto')#, cmap = 'hot_r', origin = 'lower')
            ...
            plt.savefig(directory+p+"_spectrogram_"+k+"_chem.png", dpi = 300, bbox_inches='tight', pad_inches = 0.01)
...
#            axes.flat[count].set_ylim([0.5e-6, 1])
            ...
            count = count + 1
plt.savefig(directory+"spectrogram_welch__head.png", dpi = 300, bbox_inches='tight', pad_inches = 0)
    ...
    for p in Tforfpre[1:]:
        ...
        for k in plotg:
            ...
            for t in Trial:
                ...
                axes.flat[Trial.index(t)].set_title(t)
            plt.savefig(directory+p+"_spectrogram_welch_"+k+"_chem.png", dpi = 300, bbox_inches='tight', pad_inches = 0)
    ...
    for p in Tforfpre[1:]:
        ...
        for k in plotg:
            ...
            for t in Trial:
                ...
                df0, massendtime0, masstime0, conctime0, Velocity0, Headinlettime0 = sta.calcconcmasstime(t, Het[Trial.index(t)], Anis[Trial.index(t)], gw, directory+p, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames)
                normavgconcout = np.zeros([np.shape(df0)[1],len(gvarnames)])
                normavgconcout[:,gvarnames.index(k)] = (conctime0[:,yout,gvarnames.index(k)] - np.mean(conctime0[:,yout,gvarnames.index(k)]))
                normheadin = Headinlettime0 - np.mean(Headinlettime0)
                f, Pxy = signal.csd(normavgconcout[:,gvarnames.index(k)], normheadin, fsample)
                axes.flat[Trial.index(t)].semilogy(f, np.abs(Pxy))
                axes.flat[Trial.index(t)].set_title(t)
            plt.savefig(directory+p+"_csd_welch_"+k+"_chem.png", dpi = 300, bbox_inches='tight', pad_inches = 0)
...
#            axes.flat[count].set_ylim([0.5e-6, 1])
            ...
            axes.flat[count].set_ylabel('PSD [V**2/Hz]')
            count = count + 1
plt.savefig(directory+"csd_welch__head.png", dpi = 300, bbox_inches='tight', pad_inches = 0)

#Normalized periodogram and RMS amplitude
RMSampchem, RMSampbio = sta.norm_amplitude(directory, 3, Trial, Het, Anis, gw, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames, AFbiomassvars, AFbiomassgvarnames)    
ampdf = pd.DataFrame(RMSampchem, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Max_amplitude", "Average_amplitude"])
ampdf = proc.processdataframe (ampdf, gvarnames)
discard, bth = proc.tracerstudies()
ampbth = pd.merge(ampdf, bth[['Trial', 'Regime', 'fraction', 'Time']], on = ['Trial', 'Regime'])
ampbth.to_csv("Z:/Saturated_flow/diffusion_transient/Normalized_RMSamplitude_chem.csv",sep='\t')
ampbth = pd.read_csv("Z:/Saturated_flow/diffusion_transient/Normalized_RMSamplitude_chem.csv",sep='\t')
...
dummy = stp.RMS_chem_2x2 (ampbth[ampbth["Trial"]!="52"], plotg, "Max_amplitude%", "Sensitivity", ["Medium", "Fast"])
dummy.savefig(directory+"Normalized_Maximum%_RMS_amp.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dummy = stp.RMS_chem_2x2 (ampbth[ampbth["Trial"]!="52"], plotg, "Average_amplitude%", "Average RMS amplitude")
dummy.savefig(directory+"Normalized_Average%_RMS_amp.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)

ampdf = pd.DataFrame(RMSampbio, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Max_amplitude", "Average_amplitude"])
...
Chemseries = ["Active fixed Aerobes", "Active fixed Ammonia oxidizers", "Active fixed Nitrate reducers",
                      "Active mobile Aerobes", "Active mobile Ammonia oxidizers", "Active mobile Nitrate reducers"]
ampbth = pd.read_csv("Z:/Saturated_flow/diffusion_transient/Normalized_RMSamplitude_biomass.csv",sep='\t')
ampbth["Max_amplitude%"]=ampbth["Max_amplitude"]*100
ampbth["Average_amplitude%"]=ampbth["Average_amplitude"]*100
amplitude = stp.RMSamp_biomass (ampbth[ampbth["Trial"]!="52"], Chemseries, "Max_amplitude%", "Sensitivity", ["Slow","Medium", "Fast"])
amplitude.savefig(directory+"Normalized_Maximum%_RMS_amp_biomass.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)

plotg = ["Head"]
RMSamplitude = np.zeros([len(Regimes)*len(Trial)*(len(Tforfpre)-1)*len(plotg),8])
for Reg in Regimes:
    print (Reg)
    Tforfpre = [Reg+'AR_0', Reg+'AR_1', Reg+'AR_2',Reg+'AR_5']
    for p in Tforfpre[1:]:
        ...
        for t in Trial:
            ...
            for k in range(len(plotg)):   
                ...
                idxc = Regimes.index(Reg)*(len(Tforfpre)-1)*len(Trial)*(len(plotg)) + (Tforfpre.index(p)-1)*len(Trial)*(len(plotg)) + Trial.index(t)*(len(plotg))+k
                if (t=='H'):
...
Headampdf = pd.DataFrame(RMSamplitude, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Max_amplitude", "Average_amplitude"])
Headampdf = proc.processdataframe (Headampdf, ["Head"])
discard, bth = proc.tracerstudies()
Headampbth = pd.merge(Headampdf, bth[['Trial', 'Regime', 'fraction', 'Time']], on = ['Trial', 'Regime'])
Headampbth.to_csv("Z:/Saturated_flow/diffusion_transient/RMSamplitude_Head.csv",sep='\t')
    ...
    for p in Tforfpre[1:]:
        ...
        for k in plotg:
            ...
            for t in Trial:
                print (t)
                path = directory+p+fpre+str(t)+fsuf+fpre+str(t)
                acfchem, acfbiomass, Headinlettime = sta.autocorrelation (directory, p, t, Trial, Het, Anis, gw, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames, AFbiomassvars, AFbiomassgvarnames)
                y = acfchem[np.shape(Headinlettime)[0]-1:np.shape(Headinlettime)[0]-1+300, gvarnames.index(k)]
                my_color = np.where(((y>=0.5) | (y<=-0.5)), 'orange','skyblue')
                axes.flat[Trial.index(t)].set_title(str(t))
                axes.flat[Trial.index(t)].vlines(range(300), ymin = 0, ymax = y, color = my_color, alpha = 0.4)
                axes.flat[Trial.index(t)].scatter(range(300), y, color = my_color, s= 1, alpha = 1)
                axes.flat[Trial.index(t)].set_title(t)
                axes.flat[Trial.index(t)].set_ylim((-1,1))
            plt.savefig(directory+p+"_"+k+"_chem_till300.png", dpi = 300, bbox_inches='tight', pad_inches = 0)

#Time analysis for correlation
plotg = ["Head"]
Regimes = ["Slow","Equal", "Fast"]
Headautocorr = np.zeros([len(Regimes)*len(Trial)*(len(Tforfpre)-1)*len(plotg),8])
...
#                df0, massendtime0, masstime0, conctime0, Velocity0, Headinlettime0 = sta.calcconcmasstime(t, Het[Trial.index(t)], Anis[Trial.index(t)], gw, directory+p, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames)
                ...
                normheadin = Headinlettime0 - np.mean(Headinlettime0)        
                autocorr = np.correlate(normheadin, normheadin, "full")/(np.std(Headinlettime0)*np.std(Headinlettime0)*np.shape(Headinlettime0)[0])
                idxc = Regimes.index(Reg)*(len(Tforfpre)-1)*len(Trial)*(len(plotg)) + (Tforfpre.index(p)-1)*len(Trial)*(len(plotg)) + Trial.index(t)*(len(plotg))
                if (t=='H'):
                ...
                axes.flat[count].vlines(range(int((len(autocorr)+1)/2)), ymin = 0, ymax = y, color = my_color, alpha = 0.4)
                axes.flat[count].scatter(range(int((len(autocorr)+1)/2)), y, color = my_color, s= 1, alpha = 1)
                ...
                count = count+1
plt.savefig(directory+p+"_"+k+"_4thattempt_full.png", dpi = 300, bbox_inches='tight', pad_inches = 0)
    ...
    for p in Tforfpre[1:]:
        ...
        for t in Trial:
            ...
            for k in range(len(AFbiomassgvarnames)):
                idxc = Regimes.index(Reg)*(len(Tforfpre)-1)*len(Trial)*(len(AFbiomassgvarnames)) + (Tforfpre.index(p)-1)*len(Trial)*(len(AFbiomassgvarnames)) + Trial.index(t)*(len(AFbiomassgvarnames))+k
                maxbio = np.argmax(np.abs(acfbiomass[np.shape(Headinlettime)[0]-1:, k]))
...
Ampchem = pd.DataFrame(Ampchem, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Delay", "Memory","Crosscorrelation"])
...
dfall2.to_csv("Z:/Saturated_flow/diffusion_transient/crosschor_memory_chem.csv",sep='\t')
dfall2 = pd.read_csv("Z:/Saturated_flow/diffusion_transient/crosschor_memory_chem.csv",sep='\t')
dummy = stp.correlationdistributionchem (dfall2[dfall2["Trial"]!="52"], plotg)
dummy.savefig(directory+"cross_correlation_distribution_chem.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dfall2["Norm_delay"]= dfall2["Delay"]/(dfall2["Time"]/dfall2["fraction"])
dfall2["Norm_memory"]= dfall2["Memory"]/(dfall2["Time"]/dfall2["fraction"])
dummy = stp.RMS_chem_2x2 (dfall2[dfall2["Trial"]!="52"], plotg, "Norm_memory", "Normalized Memory", ["Fast"])
dummy.savefig(directory+"Norm_memory_chem.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dummy = stp.RMS_chem_2x2 (dfall2[dfall2["Trial"]!="52"], plotg, "Memory", "Memory (days)", ["Medium","Fast"])
dummy.savefig(directory+"Memory_chem.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dummy = stp.RMS_chem_2x2 (dfall2[dfall2["Trial"]!="52"], plotg, "Norm_delay", "Normalized delay", ["Fast"])
dummy.savefig(directory+"Norm_delay_chem.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dummy = stp.RMS_chem_2x2 (dfall2[dfall2["Trial"]!="52"], plotg, "Delay", "Delay", ["Fast"])
...
Ampbio = pd.DataFrame(Ampbio, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Delay", "Memory","Crosscorrelation"])
Ampbio = proc.processdataframe (Ampbio, AFbiomassgvarnames)
bth1, bth = proc.tracerstudies()
dfallb = pd.merge(Ampbio, bth[['Trial', 'Regime', 'fraction', 'Time']], on = ['Trial', 'Regime'])
dfallb.to_csv("Z:/Saturated_flow/diffusion_transient/crosschor_memory_biomass.csv",sep='\t')
dfallb = pd.read_csv("Z:/Saturated_flow/diffusion_transient/crosschor_memory_biomass.csv",sep='\t')
dummy = stp.correlationdistributionbiomass (dfallb[dfallb["Trial"]!="52"], Chemseries)
dummy.savefig(directory+"cross_correlation_distribution_biomass.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dfallb = pd.read_csv("Z:/Saturated_flow/diffusion_transient/crosschor_memory_biomass.csv",sep='\t')
...
dummy = stp.RMSamp_biomass(dfallb[dfallb["Trial"]!="52"], Chemseries, "Norm_memory", "Normalized Memory", ["Slow"])
dummy.savefig(directory+"Norm_memory_biomass.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dummy = stp.RMSamp_biomass(dfallb[dfallb["Trial"]!="52"], Chemseries, "Memory", "Memory (days)", ["Slow", "Medium", "Fast"])
dummy.savefig(directory+"Memory_biomass.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dummy = stp.RMSamp_biomass(dfallb[dfallb["Trial"]!="52"], Chemseries, "Norm_delay", "Normalized delay")
dummy.savefig(directory+"Norm_delay_biomass.png", dpi = 300, bbox_inches = 'tight', pad = 0.01)
dummy = stp.RMSamp_biomass(dfallb[dfallb["Trial"]!="52"], Chemseries, "Delay", "Delay")
...
for Reg in Regimes:
    ...
    for p in Tforfpre[1:]:
        ...
        for t in Trial:
            ...
            plt.suptitle(Reg+p+str(t)+str(Het[Trial.index(t)])+str(Anis[Trial.index(t)]))
            for k in plotg:
                axes.flat[plotg.index(k)].stem(acfchem[np.shape(Headinlettime)[0]-1:, gvarnames.index(k)], use_line_collection = True)
...
autocov = np.zeros([len(Trial)*(numberofTempscenarios)*len(Regimes)*(len(gvarnames)),7])
crosscov = np.zeros([len(Trial)*(numberofTempscenarios)*len(Regimes)*(len(gvarnames)),7])
for Reg in Regimes:
    ...
    for p in Tforfpre:
        newd = d + p
        for t in Trial:
            df,massendtime, masstime, conctime, Velocity, head = sta.calcconcmasstime (t, Het[Trial.index(t)], Anis[Trial.index(t)], gw, newd, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames)
            for i in gvarnames:
                idxc = Regimes.index(Reg)*(len(Tforfpre)-1)*len(Trial)*(len(gvarnames)) + (Tforfpre.index(p)-1)*len(Trial)*(len(gvarnames)) + Trial.index(t)*(len(gvarnames))+gvarnames.index(i)
...
autocovdf = pd.DataFrame(autocov, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Autocovariance"])
autocovdf = proc.processdataframe (autocovdf, gvarnames)
bth1, bth = proc.tracerstudies()
dfall2 = pd.merge(autocovdf, bth[['Trial', 'Regime', 'fraction', 'Time']], on = ['Trial', 'Regime'])
dfall2.to_csv("Z:/Saturated_flow/diffusion_transient/autocovariance_chem.csv",sep='\t')
data = pd.read_csv("Z:/Saturated_flow/diffusion_transient/autocovariance_chem.csv",sep='\t')
...
#    par1 = host.twinx()
    ...
    for Reg in ["Slow","Medium","Fast"]:
        ...
        plt.scatter(y["fraction"],y["Autocovariance"], c = y["Time_series"], cmap = colseries[Regimes.index(Reg)], marker = 'o')
#        par1.scatter(y["fraction"],x["Autocovariance"], c = y["Time_series"], cmap = colseries[Regimes.index(Reg)], marker = '^')
#        par1.set_yscale("log")
        plt.yscale("log")
        plt.title(k)
        
ccovdf = pd.DataFrame(crosscov, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Crosscovariance"])
ccovdf = proc.processdataframe (ccovdf, gvarnames)
dfall2 = pd.merge(ccovdf, bth[['Trial', 'Regime', 'fraction', 'Time']], on = ['Trial', 'Regime'])
dfall2.to_csv("Z:/Saturated_flow/diffusion_transient/crosscovariance_chem.csv",sep='\t')
...
#    par1 = host.twinx()
    ...
    for Reg in ["Slow","Medium","Fast"]:
        y = dfall3[dfall3['Regime']==Reg]
        x = xall[xall['Regime']==Reg]
        plt.scatter(y["fraction"],y["Crosscovariance"], c = y["Time_series"], cmap = colseries[Regimes.index(Reg)], marker = 'o')
#        par1.scatter(y["fraction"],x["Autocovariance"], c = y["Time_series"], cmap = colseries[Regimes.index(Reg)], marker = '^')
#        par1.set_yscale("log")
        plt.yscale("log")
        plt.title(k)

mastermf = pd.DataFrame (columns = ["Trial","Variance","Anisotropy","Chem","Massflux_in","Massflux_out","Removal","Normalized_removal","UniformHet_normalized_removal","Time_series","Varyinghom_normalized_removal","Regime"])
masterbiomass = pd.DataFrame (columns = ["Trial","Variance","Anisotropy","Chem","Total_biomass_umoles","Hom_normalized_change","Unihet_normalized_change","Varyinghom_normalized_change","Time_series","species_fraction","Hom_normalized_fractionchange","Unihet_normalized_fractionchange","Varyinghom_normalized_fractionchange","Regime"])        
Regimes = ["Slow","Equal", "Fast"] 
for Reg in Regimes:
    Tforfpre = [Reg+'AR_0',Reg+'AR_1', Reg+'AR_2', Reg+'AR_5']
    mft, calcsum = sta.calcmft_temp(Tforfpre, Trial, vars, gvarnames, AFbiomassvars, AFbiomassgvarnames, directory, fpre, fsuf, Het, Anis, gw)
    dfmft = pd.DataFrame (mft, columns = ["Trial","Variance","Anisotropy","Chem","Massflux_in","Massflux_out","Removal","Normalized_removal","UniformHet_normalized_removal","Time_series","Varyinghom_normalized_removal","Regime"])
    dfmft['Regime'] = Regimes.index(Reg)
    dfbiomasssum = pd.DataFrame (calcsum, columns = ["Trial","Variance","Anisotropy","Chem","Total_biomass_umoles","Hom_normalized_change","Unihet_normalized_change","Varyinghom_normalized_change","Time_series","species_fraction","Hom_normalized_fractionchange","Unihet_normalized_fractionchange","Varyinghom_normalized_fractionchange","Regime"])
    ...
    fnamemf = "Z:/Saturated_flow/diffusion_transient/mass_flux_temporal_impact_"+Reg+".csv"
    fnamebiomasssum = "Z:/Saturated_flow/diffusion_transient/biomass_temporal_impact_"+Reg+".csv"
...
mastermf.to_csv("Z:/Saturated_flow/diffusion_transient/mass_flux_temporal_impact.csv", sep = "\t")
masterbiomass.to_csv("Z:/Saturated_flow/diffusion_transient/biomass_temporal_impact.csv", sep ="\t")
#Aggregated results
...
bth1, bth = proc.tracerstudies()
amplitudev = sta.head_amplitude (directory, nScenarios, Trial, Het, Anis, gw, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames)
corrdfv = pd.DataFrame(amplitudev, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Min","Max"])
corrdfv = proc.processdataframe (corrdfv, ["Head"])

amplitudec, conccorr, amplitudeb, conccorrb = sta.correlationanalysis(directory, nScenarios, Trial, Het, Anis, gw, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames, AFbiomassvars, AFbiomassgvarnames)
corrdf = pd.DataFrame(amplitudec, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Min","Max"])
corrdf = proc.processdataframe (corrdf, gvarnames)
corrd = pd.concat([corrdf, corrdfv], axis = 0)
dfall2 = pd.merge(corrd, bth[['Trial', 'Regime', 'fraction', 'Time']], on = ['Trial', 'Regime'])
dfall2.to_csv("Z:/Saturated_flow/diffusion_transient/amplitude%_chem.csv",sep='\t')

corrdf = pd.DataFrame(amplitudeb, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Min","Max"])
corrdf = proc.processdataframe (corrdf, AFbiomassgvarnames)
corrd = pd.concat([corrdf, corrdfv], axis = 0)
dfall2b = pd.merge(corrd, bth[['Trial', 'Regime', 'fraction','Time']], on = ['Trial', 'Regime'])
dfall2b.to_csv("Z:/Saturated_flow/diffusion_transient/amplitude%_biomass.csv",sep='\t')

corrdf = pd.DataFrame(conccorr, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Correlation","Delay"])
corrdf = proc.processdataframe (corrdf, gvarnames)
dfall2 = pd.merge(corrdf, bth[['Trial', 'Regime', 'fraction', 'Time']], on = ['Trial', 'Regime'])
...
corrdf = pd.DataFrame(conccorrb, columns = ["Trial", "Variance", "Anisotropy", "Time_series", "Regime", "Chem","Correlation","Delay"])
corrdf = proc.processdataframe (corrdf, AFbiomassgvarnames)
dfall2b = pd.merge(corrdf, bth[['Trial', 'Regime', 'fraction','Time']], on = ['Trial', 'Regime'])
dfall2b["Delayfractionofbth"] = dfall2b["Delay"]/dfall2b["Time"]
dfall2b.to_csv("Z:/Saturated_flow/diffusion_transient/correlation_biomass.csv",sep='\t')

Chemseries = ["Active fixed Aerobes", "Active fixed Ammonia oxidizers", "Active fixed Nitrate reducers",
                      "Active mobile Aerobes", "Active mobile Ammonia oxidizers", "Active mobile Nitrate reducers"]
data_biomass = pd.read_csv("Z:/Saturated_flow/diffusion_transient/amplitude%_biomass.csv",sep='\t', header = 'infer')
amplitude = stp.amplitude_biomass (data_biomass, Chemseries)
amplitude.savefig("Z:/Saturated_flow/diffusion_transient/amplitude_biomass_singletimeseries.png", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
amplitude.savefig("Z:/Saturated_flow/diffusion_transient/amplitude_biomass_singletimeseries.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0.1)

data_biomass = pd.read_csv("Z:/Saturated_flow/diffusion_transient/correlation_biomass.csv",sep='\t', header = 'infer')
corrplot, delayplot = stp.correlation_delay_biomass (data_biomass, Chemseries)
corrplot.savefig("Z:/Saturated_flow/diffusion_transient/corr_biomass.png", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
corrplot.savefig("Z:/Saturated_flow/diffusion_transient/corr_biomass.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0.1)
delayplot.savefig("Z:/Saturated_flow/diffusion_transient/delay_biomass.png", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
delayplot.savefig("Z:/Saturated_flow/diffusion_transient/delay_biomass.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0.1)

Chemseries = ["DOC", "DO", "Nitrogen", "TOC", "Head"]
data_chem = pd.read_csv("Z:/Saturated_flow/diffusion_transient/amplitude%_chem.csv",sep='\t', header = 'infer')
amplitude = stp.amplitude_chem (data_chem, Chemseries)
amplitude.savefig("Z:/Saturated_flow/diffusion_transient/amplitude_chem.png", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
amplitude.savefig("Z:/Saturated_flow/diffusion_transient/amplitude_chem.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0.1)

Chemseries = ["DOC", "Head", "Nitrogen", "TOC"]
amplitude = stp.amplitude_chem_2x2 (data_chem, Chemseries)
amplitude.savefig("Z:/Saturated_flow/diffusion_transient/amplitude_chem_square_singletimeseries.png", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
amplitude.savefig("Z:/Saturated_flow/diffusion_transient/amplitude_chem_square_singletimeseries.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0.1)

amplitudehead = stp.amplitude_head(data_chem)
data_chem = pd.read_csv("Z:/Saturated_flow/diffusion_transient/correlation_chem.csv",sep='\t', header = 'infer')
corrplot, delayplot = stp.correlation_delay_chem (data_chem, Chemseries)
corrplot.savefig("Z:/Saturated_flow/diffusion_transient/corr_chem.png", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
corrplot.savefig("Z:/Saturated_flow/diffusion_transient/corr_chem.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
delayplot.savefig("Z:/Saturated_flow/diffusion_transient/delay_chem.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0.1)
delayplot.savefig("Z:/Saturated_flow/diffusion_transient/delay_chem.png", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0.1)

corrplot, delayplot = stp.correlation_delay_chem_2x2 (data_chem, Chemseries)
corrplot.savefig("Z:/Saturated_flow/diffusion_transient/corr_chem_square.png", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
corrplot.savefig("Z:/Saturated_flow/diffusion_transient/corr_chem_square.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0)
delayplot.savefig("Z:/Saturated_flow/diffusion_transient/delay_chem_square.pdf", dpi = 300, bbox_inches = 'tight')#, pad_inches = 0.1)
...
for i in intsce:
    ...
    stp.generate_timeseries(Regimes, initseries, lastseries, Trial[Trial.index(i)], Het[Trial.index(i)], Anis[Trial.index(i)], gw, d, fpre, vars, gvarnames, fsuf, yin, yout, xleft, xright, AFbiomassvars, AFbiomassgvarnames, "Inactive mobile")
...
for Reg in Regimes:
    ...
    for k in intsce:
        ...
        for time in timepoints:
            title = "Variance "+str(Het[Trial.index(k)])+" : Anisotropy "+str(Anis[Trial.index(k)])+" at Time: "+str(time*5)+" days"
            heatmapc = stp.heatmapconcdist_temp (df, hmvars, k, hmgvarnames, d, fpre, title, time)
            picname = "Z:/Saturated_flow/diffusion_transient/"+Reg+str(k)+ "_" + "heatmap" + "_conc_"+str(time*5)+".png"
#            pictosave = heatmapc.get_figure()
            heatmapc.savefig(picname, dpi = 300)    
            heatmapb = stp.heatmapconcdist_temp (df, hmbvars, k, hmbgvarnames, d, fpre, title, time)     
            picname = "Z:/Saturated_flow/diffusion_transient/"+Reg+str(k)+ "_" + "heatmap" + "_biomass_"+str(time*5)+".png"
