...
for Reg in ["Slow", "Equal", "Fast"]:
    ...
    fpre = "RF-A"
    for j in range(len(Trial)):
        df, conctime, Velocity, head = uta.calcconcmasstime(
            Trial[j],
            Het[j],
            Anis[j],
            gw,
...
for Reg, step in zip(Regimes, steps):
    ...
    fpre = "NS-A"
    df, massendtime, masstime, conctime, Velocity, head = uta.calcconcmasstime(
...
for idx in range(len(dfall2)):
    writer.writerow(
        [
            ...
            dfall2["Trial"][idx],
            dfall2["Variance"][idx],
            dfall2["Anisotropy"][idx],
            gvarnames[int(dfall2["Chem"][idx])],
            dfall2["Inlet_total_mass_flux"][idx],
            dfall2["Outlet_mass_flux"][idx],
            dfall2["delmassflux"][idx],
            dfall2["del2massflux"][idx],
            dfall2["Breakthroughtime"][idx],
            dfall2["fraction"][idx],
            dfall2["Regime"][idx],
        ]
    )
...
dummy.savefig(
    "X:/Saturated_flow/diffusion/steadystate_impact_massflux.png",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0,
)
dummy.savefig(
    "X:/Saturated_flow/diffusion/steadystate_impact_massflux.pdf",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0.1,
)
...
for idx in range(len(dfall2)):
    writer.writerow(
        [
            ...
            dfall2["Variance"][idx],
            dfall2["Anisotropy"][idx],
            AFbiomassgvarnames[int(dfall2["Chem"][idx])],
            dfall2["Total_biomass"][idx],
            dfall2["fractionoftotal"][idx],
            dfall2["Change_umoles"][idx],
            dfall2["Breakthroughtime"][idx],
            dfall2["fraction"][idx],
            dfall2["Regime"][idx],
        ]
...
for j in Trial:
    ...
    print(
        ...
        ": last time step: ",
        (np.shape(df)[1] - 1) * 5,
        5475 - (np.shape(df)[1] - 1) * 5,
    )
    for h in Headers[3:]:
        fname = (
            "X:/Saturated_flow/changedkindox_transient/"
            + Reg
            + "AR_1/NS-A"
            + str(j)
            + fsuf
            + "model_MASS_TRANSPORT_"
            + h[2:-1]
            + "_primary_value.asc"
        )
        ...
        for y in range(51):
            for x in range(31):
                writer.writerow(
                    [
                        df[Headers.index(h) - 3, -1, yout - y, x],
                        df[Headers.index(h) - 3, -1, yout - y, x],
                    ]
                )
        csvfile.close()
