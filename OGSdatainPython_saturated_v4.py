...
biomasssum = sssa.calcsum (Trial, Het, Anis, gw, directory, fpre, fsuf, yin, yout, xleft, xright, AFbiomassvars, AFbiomassgvarnames)
meanresultsarr, stdresultsarr, covresultsarr = sssa.calcaggres (mf)

#Writing the results into csv files
fnamemf = directory+fpre+str(Trial[0])+"_"+str(Trial[-1])+"_mass_flux"+Reg+str(gw) + datetime.now().strftime("%d%m%Y%H%M")+".csv"
csvfile= open(fnamemf, "w")
writer = csv.writer(csvfile, delimiter='\t', quotechar='\t', quoting=csv.QUOTE_MINIMAL, lineterminator = '\n')
writer.writerow(["Sno","Trial", "Variance", "Anisotropy", "Chem", "Inlet_total_mass_flux", "Outlet_mass_flux", "delmassflux", "del2massflux", "fdelmassflux"])
for j in Trial:
    for i in gvarnames:
        idx = Trial.index(j)*len(gvarnames)+gvarnames.index(i)
        writer.writerow([idx+1, j, Het[Trial.index(j)], Anis [Trial.index(j)], i, mf[idx,4], mf[idx,5], mf[idx,6], mf[idx,7], mf[idx,8]])
csvfile.close()
...
for j in range(len(Trial)):
        ...
        for i in range(len(gvarnames)):
            ...
            if (gvarnames[i]=="Nitrogen"):
               ...
               noutlet = 0
               for n in Nspecies:
                   ninlet = ninlet + df[n-3,-1,yin,xleft]*satiledg*(diff+disp*abs(veliledg))+df[n-3,-1,yin,xright]*satiredg*(diff+disp*abs(veliredg))+sum(df[n-3,-1,yin,xleft+1:xright]*satielem*(diff+disp*abs(velielem)))+abs(df[n-3,-1,yin,xleft]*satiledg*veliledg*vedge + df[n-3,-1,yin,xright]*satiredg*veliredg*vedge + sum(df[n-3,-1,yin,xleft+1:xright]*satielem*velielem*velem))
                   noutlet = noutlet + df[n-3,-1,yout,xleft]*satoledg*(diff+disp*abs(veloledg))+df[n-3,-1,yout,xright]*satoredg*(diff+disp*abs(veloredg))+sum(df[n-3,-1,yout,xleft+1:xright]*satoelem*(diff+disp*abs(veloelem))) + abs(df[n-3,-1,yout,xleft]*satoledg*veloledg*vedge + df[n-3,-1,yout,xright]*satoredg*veloredg*vedge + sum(df[n-3,-1,yout,xleft+1:xright]*satoelem*veloelem*velem))
               ninlet = ninlet/5 +  df[Amm1-3,-1,yin,xleft]*satiledg*(diff+disp*abs(veliledg))+df[Amm1-3,-1,yin,xright]*satiredg*(diff+disp*abs(veliredg))+sum(df[Amm1-3,-1,yin,xleft+1:xright]*satielem*(diff+disp*abs(velielem)))+ df[nitra1-3,-1,yin,xleft]*satiledg*(diff+disp*abs(veliledg))+df[nitra1-3,-1,yin,xright]*satiredg*(diff+disp*abs(veliredg))+sum(df[nitra1-3,-1,yin,xleft+1:xright]*satielem*(diff+disp*abs(velielem)))+ (df[Amm1-3,-1,yin,xleft]*satiledg*veliledg*vedge + df[Amm1-3,-1,yin,xright]*satiredg*veliredg*vedge + sum(df[Amm1-3,-1,yin,xleft+1:xright]*satielem*velielem*velem) + df[nitra1-3,-1,yin,xleft]*satiledg*veliledg*vedge + df[nitra1-3,-1,yin,xright]*satiredg*veliredg*vedge + sum(df[nitra1-3,-1,yin,xleft+1:xright]*satielem*velielem*velem))
               noutlet = noutlet/5 + df[Amm1-3,-1,yout,xleft]*satoledg*(diff+disp*abs(veloledg))+df[Amm1-3,-1,yout,xright]*satoredg*(diff+disp*abs(veloredg))+sum(df[Amm1-3,-1,yout,xleft+1:xright]*satoelem*(diff+disp*abs(veloelem)))+df[nitra1-3,-1,yout,xleft]*satoledg*(diff+disp*abs(veloledg))+df[nitra1-3,-1,yout,xright]*satoredg*(diff+disp*abs(veloredg))+sum(df[nitra1-3,-1,yout,xleft+1:xright]*satoelem*(diff+disp*abs(veloelem)))+(df[Amm1-3,-1,yout,xleft]*satoledg*veloledg*vedge + df[Amm1-3,-1,yout,xright]*satoredg*veloredg*vedge + sum(df[Amm1-3,-1,yout,xleft+1:xright]*satoelem*veloelem*velem) + df[nitra1-3,-1,yout,xleft]*satoledg*veloledg*vedge + df[nitra1-3,-1,yout,xright]*satoredg*veloredg*vedge + sum(df[nitra1-3,-1,yout,xleft+1:xright]*satoelem*veloelem*velem))
               ...
               mf[idx,5] = abs(noutlet)#/(sum(veloelem)*velem + (veloledg+veloredg)*vedge)
                ...
                coutlet = 0
                for c in Cspecies:
                    cinlet  = cinlet +  df[c-3,-1,yin,xleft]*satiledg*(diff+disp*abs(veliledg))+df[c-3,-1,yin,xright]*satiredg*(diff+disp*abs(veliredg))+sum(df[c-3,-1,yin,xleft+1:xright]*satielem*(diff+disp*abs(velielem)))+abs(df[c-3,-1,yin,xleft]*satiledg*veliledg*vedge + df[c-3,-1,yin,xright]*satiredg*veliredg*vedge + sum(df[c-3,-1,yin,xleft+1:xright]*satielem*velielem*velem))
                    coutlet = coutlet +  df[c-3,-1,yin,xleft]*satiledg*(diff+disp*abs(veliledg))+df[c-3,-1,yin,xright]*satiredg*(diff+disp*abs(veliredg))+sum(df[c-3,-1,yin,xleft+1:xright]*satielem*(diff+disp*abs(velielem)))+abs(df[c-3,-1,yout,xleft]*satoledg*veloledg*vedge + df[c-3,-1,yout,xright]*satoredg*veloredg*vedge + sum(df[c-3,-1,yout,xleft+1:xright]*satoelem*veloelem*velem))
...
#                mf[idx,5] = (df[vars[i]-3,-1,yout-1,xleft]-df[vars[i]-3,-1,yout,xleft])*satoledg*(diff+disp*abs(veloledg))+(df[vars[i]-3,-1,yout-1,xright]-df[vars[i]-3,-1,yout,xright])*satoredg*(diff+disp*abs(veloredg))+sum((df[vars[i]-3,-1,yout-1,xleft+1:xright]-df[vars[i]-3,-1,yout,xleft+1:xright])*satoelem*(diff+disp*abs(veloelem)))+abs(((df[vars[i]-3,-1,yout,xleft]*satoledg*veloledg*vedge + df[vars[i]-3,-1,yout,xright]*satoredg*veloredg*vedge + sum(df[vars[i]-3,-1,yout,xleft+1:xright]*satoelem*veloelem*velem))))/por#/(sum(veloelem)*velem + (veloledg+veloredg)*vedge)
                mf[idx,4] = (df[vars[i]-3,-1,yin,xleft]-df[vars[i]-3,-1,yin+1,xleft])*satiledg*disp+(df[vars[i]-3,-1,yin,xright]-df[vars[i]-3,-1,yin+1,xright])*satiredg*disp+sum((df[vars[i]-3,-1,yin,xleft+1:xright]-df[vars[i]-3,-1,yin+1,xleft+1:xright])*satielem*disp)+abs(((df[vars[i]-3,-1,yin,xleft]*satiledg*veliledg*vedge + df[vars[i]-3,-1,yin,xright]*satiredg*veliredg*vedge + sum(df[vars[i]-3,-1,yin,xleft+1:xright]*satielem*velielem*velem))))/por#/(sum(velielem)*velem + (veliledg+veliredg)*vedge)
                mf[idx,5] = (df[vars[i]-3,-1,yout-1,xleft]-df[vars[i]-3,-1,yout,xleft])*satoledg*disp+(df[vars[i]-3,-1,yout-1,xright]-df[vars[i]-3,-1,yout,xright])*satoredg*disp+sum((df[vars[i]-3,-1,yout-1,xleft+1:xright]-df[vars[i]-3,-1,yout,xleft+1:xright])*satoelem*disp)+abs(((df[vars[i]-3,-1,yout,xleft]*satoledg*veloledg*vedge + df[vars[i]-3,-1,yout,xright]*satoredg*veloredg*vedge + sum(df[vars[i]-3,-1,yout,xleft+1:xright]*satoelem*veloelem*velem))))/por#/(sum(veloelem)*velem + (veloledg+veloredg)*vedge)
            mf[idx,6] = (mf[idx,4]-mf[idx,5])

f = "X:/massbalance_file1_"+Reg+".csv"
csvfile= open(f, "w")
writer = csv.writer(csvfile, delimiter='\t', quotechar='\t', quoting=csv.QUOTE_MINIMAL, lineterminator = '\n')
writer.writerow(["Sno","Trial", "Variance", "Anisotropy", "Chem", "Inlet_massflux_umolesperday", "Outlet_massflux_umolesperday", "Moles_removed_per_day"])
for j in range(len(Trial)):
    for i in range(len(gvarnames)):
        idx = j*len(gvarnames)+i
        writer.writerow([idx, Trial[j],Het[j],Anis[j],gvarnames[i],mf[idx,4], mf[idx,5],mf[idx,6]])
csvfile.close()
...
for Reg in Regimes:
    ...
    fpre = "NS-A"
    mf = sssa.calcmassflux (Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames)
    biomasssum = sssa.calcsum (Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, AFbiomassvars, AFbiomassgvarnames)
...
dfallbiomass = pd.DataFrame(data = masterbiomasssum, columns = ["Trial", "Variance", "Anisotropy", "Chem", "Total_biomass", "Change_umoles", "fractionoftotal","del2biomass", "Regime"])
...
dfall2.to_csv("Z:/Saturated_flow/diffusion_transient/massflux_withbreakthrough_forMartin_v4_complete.csv",sep='\t')
...
dummy.savefig("Z:/Saturated_flow/diffusion_transient/steadystate_impact_massflux.png", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
dummy.savefig("Z:/Saturated_flow/diffusion_transient/steadystate_impact_massflux.pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0.1)
dummy = sssp.norm_mf_2x2 (dfall2, gvarnames)
dummy.savefig("Z:/Saturated_flow/diffusion_transient/steadystate_impact_massflux_square.png", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
dummy.savefig("Z:/Saturated_flow/diffusion_transient/steadystate_impact_massflux_square.pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0.1)


dfall2biomass = pd.merge(dfallbiomass2, breakthrough[['Trial', 'Regime', 'Time', 'fraction','%ofhomogeneous']], on = ['Trial', 'Regime']).rename(columns={'Time':'Breakthroughtime'})
dfall2biomass["%del2biomass"] = dfall2biomass["del2biomass"]*100
dfall2biomass.to_csv("Z:/Saturated_flow/diffusion_transient/biomass_withbreakthrough_forMartin_v4_complete.csv",sep='\t')
...
dummy.savefig("Z:/Saturated_flow/diffusion_transient/steadystate_active_biomass.png", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
dummy.savefig("Z:/Saturated_flow/diffusion_transient/steadystate_active_biomass.pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0.1)

dummy = sssp.steadystate_active_biomass (dfall2biomass, inactivebiomassgvarnames)
dummy.savefig("Z:/Saturated_flow/diffusion_transient/steadystate_inactive_biomass.png", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
dummy.savefig("Z:/Saturated_flow/diffusion_transient/steadystate_inactive_biomass.pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0.1)

fnamebiosum = directory+fpre+"Anaerobic"+str(Trial[0])+"_"+str(Trial[-1])+"_"+"biomass_"+str(gw) + datetime.now().strftime("%d%m%Y%H%M")+".csv"
...
fnamemf_summary = directory+fpre+"Anaerobic"+str(Trial[0])+"_"+str(Trial[-1])+"_"+"new_MF_"+str(gw) +datetime.now().strftime("%d%m%Y%H%M")+ "_summary.csv"
...
intsce = Trial
sssp.plotconcallatss(Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames, intsce, colors, nrows, ncols, figsize)
...
chem = "DO"
Regimes = ["Slow", "Equal", "Fast"]
dummy = sssp.plotconcwithhet (Regimes, intsce, chem, colorseries, Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames)
picname1 = "X:/Saturated_flow/diffusion/"+str(intsce[0])+ "_" + str(intsce[-1])+"_"+datetime.now().strftime("%d%m%Y%H%M")+"_"+chem+"with_het.pdf"
picname2 = "X:/Saturated_flow/diffusion/"+str(intsce[0])+ "_" + str(intsce[-1])+"_"+datetime.now().strftime("%d%m%Y%H%M")+"_"+chem+"with_het.png"
...
#        a.set_title (Reg+" flow", fontsize = 15)
        a.axis('off')
        plt.savefig("//tsclient/D/Saturated_flow/diffusion/heterogeneous_profiles_"+ctype+".pdf", dpi = 300)
        plt.savefig("//tsclient/D/Saturated_flow/diffusion/heterogeneous_profiles_"+ctype+".png", dpi = 300)
...
for ctype in ["chem"]:
    ...
    for a, Reg in zip(ax, Regimes):
        ...
        if Reg == "Equal":
            ...
            a.set_title (Reg+" flow", fontsize = 15)
        a.axis('off')
        plt.savefig("//tsclient/D/Saturated_flow/diffusion/homogeneous_profiles_"+ctype+".pdf", dpi = 300)
        plt.savefig("//tsclient/D/Saturated_flow/diffusion/homogeneous_profiles_"+ctype+".png", dpi = 300)
...
○
sssp.concprofileatssX(Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames, intsce, colors)
sssp.concprofileatssX(Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, AFbiomassvars, AFbiomassgvarnames, intsce, colors)
...
nrows = 2
ncols = 2
sssp.plottimeseries(Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames, intsce, inttime, colors, nrows, ncols, figsize, chem, lylim, uylim)
...
for Reg in Regimes:
    ...
    fig = sssp.plotoxiccellssdo(20, intsce, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars)
    picname = directory+"DOandoxiccells_"+datetime.now().strftime("%d%m%Y%H%M")+".png"
    fig.savefig(picname, dpi = 300, bbox_inches = 'tight', pad_inches = 0)
    picname = directory+"DOandoxiccells_"+datetime.now().strftime("%d%m%Y%H%M")+".pdf"
    fig.savefig(picname, dpi = 300, bbox_inches = 'tight', pad_inches = 0)
...
for Reg in Regimes:
    ...
    for k in intsce:
        ...
        heatmapb = sssp.heatmapconcdistLOG (df, AFbiomassvars, k, AFbiomassgvarnames, d, fpre, title) 
...
#            a.set_title (Reg+" flow", fontsize = 15)
            a.axis('off')
        if (Reg == "Equal"):
            ax[Regimes.index(Reg),0].annotate("Medium flow", xy = (-0.04, 0.4), xytext = (0,0), xycoords = 'axes fraction', textcoords='offset points', size = 'large', ha = 'left', va = 'center', rotation = 'vertical', fontsize = 15)
        else:
            ax[Regimes.index(Reg),0].annotate(Reg + " flow", xy = (-0.04, 0.4), xytext = (0,0), xycoords = 'axes fraction', textcoords='offset points', size = 'large', ha = 'left', va = 'center', rotation = 'vertical', fontsize = 15)
    plt.subplots_adjust(wspace=0.025, hspace=0.025)
    plt.savefig("X:/Saturated_flow/diffusion/1D_heterogeneous_profiles_"+ctype+".pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
    plt.savefig("X:/Saturated_flow/diffusion/1D_heterogeneous_profiles_"+ctype+".png", dpi = 300, bbox_inches = 'tight', pad_inches = 0.1)
...
#            a.set_title (Reg+" flow", fontsize = 15)
            a.axis('off')
        if (Reg == "Equal"):
            ax[Regimes.index(Reg),0].annotate("Medium flow", xy = (0, 0.5), xytext = (0,0), xycoords = 'axes fraction', textcoords='offset points', size = 'large', ha = 'left', va = 'center', rotation = 'vertical', fontsize = 15)
        else:
            ax[Regimes.index(Reg),0].annotate(Reg + " flow", xy = (0, 0.5), xytext = (0,0), xycoords = 'axes fraction', textcoords='offset points', size = 'large', ha = 'left', va = 'center', rotation = 'vertical', fontsize = 15)
    plt.subplots_adjust(wspace=0.025, hspace=0.025)
    plt.savefig("X:/Saturated_flow/diffusion/2D_heterogeneous_profiles_"+ctype+".pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
    plt.savefig("X:/Saturated_flow/diffusion/2D_heterogeneous_profiles_"+ctype+".png", dpi = 300, bbox_inches = 'tight', pad_inches = 0.1)

#Checking total fixed microbes vs carrying capacity
Fbiomassvars = [Bfo1, Bfa1, Bfn1, Bifo1, Bifa1, Bifn1]
Fbiomassgvarnames = ["Active fixed Aerobes", "Active fixed Ammonia oxidizers", "Active fixed Nitrate reducers",
                      "Inactive fixed Aerobes", "Inactive fixed Ammonia oxidizers", "Inactive fixed Nitrate reducers"]
for Reg in Regimes:
    d = r"Z:/Saturated_flow/diffusion_transient/"+ Reg+ "AR_0/"
    for j in intsce:    
        picture = sssp.biomassvscarryingcapacity(j, Het[Trial.index(j)], Anis[Trial.index(j)], gw, d, fpre, fsuf, yin, yout, xleft, xright, Fbiomassvars)
        picture.savefig("//tsclient/D/Saturated_flow/diffusion/"+Reg+str(j)+"_biomassvscarryingcapacity.png", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
        picture.savefig("//tsclient/D/Saturated_flow/diffusion/"+Reg+str(j)+"_biomassvscarryingcapacity.pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
start = 0
end = 15
chem = "DOC"
...
for j in range(len(Trial)):
    df,massendtime, masstime, conctime, Velocity = sssa.calcconcmasstime (Trial[j], Het[j], Anis[j], gw, d, fpre, fsuf, yin, yout, xleft, xright, vars)
...
for y in range(51):
    writer.writerow([y, df[0,-1,y,15],df[1,-1,y,15],df[2,-1,y,15],df[3,-1,y,15],df[4,-1,y,15],df[5,-1,y,15],df[6,-1,y,15],df[7,-1,y,15],df[8,-1,y,15],df[9,-1,y,15],df[10,-1,y,15],
                     df[11,-1,y,15],df[12,-1,y,15],df[13,-1,y,15],df[14,-1,y,15],df[15,-1,y,15],df[16,-1,y,15],df[17,-1,y,15],df[18,-1,y,15],df[19,-1,y,15],df[20,-1,y,15],
                     df[21,-1,y,15],df[22,-1,y,15],df[23,-1,y,15],df[24,-1,y,15],df[25,-1,y,15],df[26,-1,y,15]])
csvfile.close()
...
picname = "X:/Saturated_flow/diffusion/flux_onlyvariance_"+datetime.now().strftime("%d%m%Y%H%M")+".png"
