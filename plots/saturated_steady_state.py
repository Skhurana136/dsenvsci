...
from analyses.saturated_transient import calcconcmasstime, biomasstimefunc, calcconcmasstimeX
...
#                axes.flat[colidx1].set_xlabel('Fraction of breakthrough time in base case', fontsize = 15)
    plt.legend(loc = "best", fontsize = 12)
    plt.annotate("Normalized reduction in mass flux in the domain", xy = (-1.2, 1), xytext = (- 80,0), xycoords = 'axes fraction', textcoords='offset points', size = 'large', ha = 'left', va = 'center', rotation = 'vertical', fontsize = 15)
    plt.annotate('Fraction of breakthrough time in base case', xy = (-0.7,0), xytext = (0,-50), xycoords = 'axes fraction', textcoords='offset points', size = 'large', ha = 'left', va = 'center', fontsize = 15)
...
#    title = "Variance "+str(Het[Trial.index(k)])+" : Anisotropy "+str(Anis[Trial.index(k)])
        vel = data[2,-1,:,:]*-1
        sns.heatmap(vel, square = False, cmap = "YlGnBu", xticklabels=False, yticklabels=False, ax=ax.flat[trialseries.index(k)],
...
#        ax.flat[trialseries.index(k)].set_title("Variance: " + str(varianceseries[trialseries.index(k)]) + "& Anisotropy: "+str(anisotropyseries[trialseries.index(k)]), fontsize = subtitlesize)
        ax.flat[trialseries.index(k)].set_title(str(varianceseries[trialseries.index(k)]) + ":"+str(anisotropyseries[trialseries.index(k)]), fontsize = subtitlesize)
    return (plt)
...
def concprofilewithH (Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, cvars, gvarnames, intsce, colors,i, averagingtype):
    legendsize = 16
    axissize = 16
    ticksize = 14
    titlesize = 20
    if (('DOC' in gvarnames) & (averagingtype == "flux")):
        df,massendtime, masstime, conctime, Velocity, head = calcconcmasstime (Trial[Trial.index(i)], Het[Trial.index(i)], Anis[Trial.index(i)], gw, d, fpre, fsuf, yin, yout, xleft, xright, cvars, gvarnames)
        dfh,massendtimeh, masstimeh, conctimeh, Velocityh, headh = calcconcmasstime (Trial[Trial.index('H')], Het[Trial.index('H')], Anis[Trial.index('H')], gw, d, fpre, fsuf, yin, yout, xleft, xright, cvars, gvarnames)
        yindex = list(range(51))
        fig, host = plt.subplots()
        fig.subplots_adjust(top=0.8)
        ...
        host.legend(lines, [l.get_label() for l in lines], bbox_to_anchor = (1.9,0.5),fontsize = legendsize, loc = "center")
        plt.title ("Variance "+str(Het[Trial.index(i)])+" : Anisotropy "+str(Anis[Trial.index(i)]), fontsize = titlesize, pad = 100)
        plt.tight_layout()
        ...
        df,massendtime, masstime, bioconctime, Velocity, head = calcconcmasstime (Trial[Trial.index(i)], Het[Trial.index(i)], Anis[Trial.index(i)], gw, d, fpre, fsuf, yin, yout, xleft, xright, cvars, gvarnames)
        dfh,massendtimeh, masstimeh, bioconctimeh, Velocityh, headh = calcconcmasstime ('H', Het[Trial.index('H')], Anis[Trial.index('H')], gw, d, fpre, fsuf, yin, yout, xleft, xright, cvars, gvarnames)
        yindex = list(range(51))
        fig, host = plt.subplots()
        fig.subplots_adjust(top=0.8)
