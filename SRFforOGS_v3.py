...
counter = 64 #Fourth realization with an extra variance of 5 which was not included earlier
    ...
    for j in range(len(ls)):
        for k in range(len(an)):
            model = TPLExponential(
                    ...
                    len_scale=ls[j],   # length scale (a.k.a. range), len_up = len_low + len_scale
...
#            plt.close()
            perm = 2*np.exp(field)
            print(pref+str(counter), v[i], ls[j], an[k], np.min(perm), np.mean(perm), np.max(perm), np.std(perm))

            fname = pref+str(counter)+randomkeyword+extnc
            permfile = np.arange(perm.size).reshape((x.size, y.size))  # Contains f(x, y)
