...
fnamemf = d+fpre+str(Trial[0])+"_"+str(Trial[-1])+"_mass_flux"+Reg+str(gw) + datetime.now().strftime("%d%m%Y%H%M")+".csv"
...
#masterbiomasssummob = np.zeros([1,7])
Regimes = ["Slow", "Equal", "Fast"]
for Reg in Regimes:
    d = "X:/Saturated_flow/diffusion/"+ Reg+ "AR_changedkindox/"
    fpre = "NS-A"
    mf = sk.calcmassflux (Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames)
    biomasssum = sk.calcsum (Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, AFbiomassvars, AFbiomassgvarnames)
#    biomasssummob = sk.calcsum (Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, AMbiomassvars, AMbiomassgvarnames)
...
#del2biomassmob = np.zeros([len(Trial)*len(Regimes)*len(AMbiomassgvarnames),1])
...
#dfallbiomassmob = pd.DataFrame(data = masterbiomasssummob, columns = ["Trial", "Variance", "Anisotropy", "Chem", "Total_biomass", "Change_umoles", "fractionoftotal","del2biomass", "Regime"])
...
dfallmf["Trial"] = dfallmf["Trial"].replace([0], "H")
...
dfallbiomass["Trial"] = dfallbiomass["Trial"].replace([0], "H")
...
for k in Chems:
    dfc = dfall2[dfall2['Chem'].astype(int)==gvarnames.index(k)]
...
#    print (line)
    for i in Regimes:
        ...
        print (i)
        axes.flat[colidx1].scatter("fraction", "del2massflux", color = colseries[Regimes.index(i)], data = dfcr, label = i + " flow")
...
plt.annotate("Normalized reduction in mass flux in the domain", xy = (0, 2.2), xytext = (- 80,0), xycoords = 'axes fraction', textcoords='offset points', size = 'large', ha = 'left', va = 'center', rotation = 'vertical', fontsize = 15)
fig.savefig("X:/Saturated_flow/diffusion/steadystate_impact_massflux.png", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
fig.savefig("X:/Saturated_flow/diffusion/steadystate_impact_massflux.pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0.1)
...
ncols = len(Regimes)
nrows = len(Chems)
fig, axes = plt.subplots(nrows = 4, ncols = 3, figsize = [len(Chems)*1.5,len(Chems)*1.5])
...
#    print (line)
    for i in Regimes:
        ...
        dfcr = dfctemp[dfctemp['Regime']==i]
        print (i)
        axes.flat[colidx1].scatter("fraction", "Change_umoles", color = colseries[Regimes.index(i)], data = dfcr, label = i+ " flow")
#        axes.flat[colidx1].plot(pred, slope + intercept*pred, '--')
#        axes.flat[colidx1].set_ylabel(k, fontsize = 15)
        axes.flat[colidx1].tick_params(axis = 'y', labelsize = 15)
...
plt.annotate("Total biomass in the domain (uM C)", xy = (0, 2.2), xytext = (-800,0), xycoords = 'axes fraction', textcoords='offset points', size = 'large', ha = 'left', va = 'center', rotation = 'vertical', fontsize = 15)
...
fig.savefig("X:/Saturated_flow/diffusion/steadystate_biomass.png", dpi = 300, bbox_inches = 'tight', pad_inches = 0)
fig.savefig("X:/Saturated_flow/diffusion/steadystate_biomass.pdf", dpi = 300, bbox_inches = 'tight', pad_inches = 0.1)
...
for Reg in Regimes:
    d = r"X:/Saturated_flow/diffusion/"+ Reg+ "AR_changedkindox/"
    for i in intsce:
        df = np.load(d+fpre+str(i)+fsuf+fpre+str(i)+"_df.npy")
#        print(np.shape(df), np.max(df[Bfn1-3, -1, :,:]))
        chemspecies = sk.concprofilewithH(Trial, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars, gvarnames, intsce, colors, i)
        picname = d+fpre+str(i)+ "_" +"_chem_conc_withH.png"
        chemspecies.savefig(picname, bbox_inches = 'tight', dpi = 300)
        ...
        picname = d+fpre+str(i)+ "_" +"_biomass_conc_withH.png"
        biodist.savefig(picname, bbox_inches = 'tight', dpi = 300)
...
for Reg in Regimes:
    d = r"X:/Saturated_flow/diffusion/"+ Reg+ "AR_changedkindox/"
    fig = sk.plotoxiccellssdo(20, intsce, Het, Anis, gw, d, fpre, fsuf, yin, yout, xleft, xright, vars)
    picname = d+"DOandoxiccells_"+datetime.now().strftime("%d%m%Y%H%M")+".png"
    fig.savefig(picname, dpi = 300, bbox_inches = 'tight', pad_inches = 0)
    picname = d+"DOandoxiccells_"+datetime.now().strftime("%d%m%Y%H%M")+".pdf"
    fig.savefig(picname, dpi = 300, bbox_inches = 'tight', pad_inches = 0)
...
for Reg in Regimes:
    ...
    for k in intsce:
        ...
        heatmapc = sk.heatmapconcdist (df, vars, k, gvarnames, d, fpre, title)
        heatmapb = sk.heatmapconcdist (df, AFbiomassvars, k, AFbiomassgvarnames, d, fpre, title)       
...
for Reg in Regimes:
    d = r"Z:/Saturated_flow/diffusion_transient/"+ Reg+ "AR_0/"
    for j in intsce:    
        picture = sk.biomassvscarryingcapacity(j, Het[Trial.index(j)], Anis[Trial.index(j)], gw, d, fpre, fsuf, yin, yout, xleft, xright, Fbiomassvars)
...
dummy = sk.boxV_Aflux (fastc, slowc, equalc, sgvarnames, imgsize = [24,18])
