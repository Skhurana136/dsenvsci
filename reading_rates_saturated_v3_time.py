    ...
    for k in range(len(Tforfpre)):
        ...
        for j in range(len(Trial)):
            ...
            for i in range(len(respindx)):
                idx = (
                    Regimes.index(Reg) * len(Tforfpre) * len(Trial) * len(respindx)
                    + k * len(Trial) * len(respindx)
                    + j * len(respindx)
                    + i
                )
                summ[idx, 6] = sum(M[:, i])
...
dfall2.to_csv(
    "Z:/Saturated_flow/diffusion_transient/total_rate_withbreakthrough.csv", sep="\t"
)
        ...
        for k in range(len(respindx)):
            ...
            writer.writerow(
                [
                    ...
                    gratenames[k],
                    summ[idx, k],
                    delsummresprate[idx, k],
                    delsummhetresprate[idx, k],
                    i,
                ]
            )
csvfile.close()
