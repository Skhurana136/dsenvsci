...
for j in range(len(Trial)):    
    ...
    df = np.load(di+fpre+str(Trial[j])+"_df.npy")
    for resp in range(Numberofrates):
        totalsum = df[Bfo1, np.shape(df)[1]-1, :, :]+df[Bifo1, np.shape(df)[1]-1, :, :]+df[Bfn1, np.shape(df)[1]-1, :, :]+ df[Bifn1, np.shape(df)[1]-1, :, :]+df[Bfa1, np.shape(df)[1]-1, :, :]+df[Bifa1, np.shape(df)[1]-1, :, :]+df[Bfs1, np.shape(df)[1]-1, :, :]+ df[Bifs1, np.shape(df)[1]-1, :, :]
